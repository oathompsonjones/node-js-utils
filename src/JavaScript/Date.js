...
Date.prototype.toUptimeString = function (times = []) {
    const weeks = Math.floor((this.getUTCDate() - 1) / 7);
    if (!times.length) {
        if ((this.getUTCFullYear() - 1970) > 0)
            return `${this.getUTCFullYear() - 1970}y, ${this.getUTCMonth()}mo, ${weeks}w, ${(this.getUTCDate() - 1) % 7}d`;
        if (this.getUTCMonth() > 0)
            return `${this.getUTCMonth()}mo, ${weeks}w, ${(this.getUTCDate() - 1) % 7}d, ${this.getUTCHours()}h`;
        if (weeks > 0)
            return `${weeks}w, ${(this.getUTCDate() - 1) % 7}d, ${this.getUTCHours()}h`;
        if (((this.getUTCDate() - 1) % 7) > 0)
            return `${this.getUTCDate() - 1}d, ${this.getUTCHours()}h, ${this.getUTCMinutes()}m`;
        if (this.getUTCHours() > 0)
            return `${this.getUTCHours()}h, ${this.getUTCMinutes()}m, ${this.getUTCSeconds()}s`;
        if (this.getUTCMinutes() > 0)
            return `${this.getUTCMinutes()}m, ${this.getUTCSeconds()}s, ${this.getUTCMilliseconds()}ms`;
        if (this.getUTCSeconds() > 0)
            return `${this.getUTCSeconds()}s, ${this.getUTCMilliseconds()}ms`;
